...
from software.data import get_test_loader, CIFAR10_mean, CIFAR10_std, SVHN_mean, SVHN_std
      ...
      for j, loader in enumerate([train_loader, valid_loader, test_loader, random_loader]):
        ...
        for k, image in enumerate(input):
          ...
          if "mnist" in args.dataset:
            ...
            means = np.array(CIFAR10_mean).reshape((3, 1, 1))
            stds = np.array(CIFAR10_std).reshape((3, 1, 1))
            image = (image.numpy()*stds)+means
            plt.imshow(np.transpose(image, (1, 2, 0)))
          elif "svhn" in args.dataset:
            means = np.array(SVHN_mean).reshape((3, 1, 1))
            stds = np.array(SVHN_std).reshape((3, 1, 1))
            image = (image.numpy()*stds)+means
            plt.imshow(np.transpose(image, (1, 2, 0)))
          plt.axis('off')
      plt.tight_layout()
